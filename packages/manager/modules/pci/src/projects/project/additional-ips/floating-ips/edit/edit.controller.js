  ...
  loadInstances() {
    ...
    this.$q
      .all({
        ...
        associatedInstance: this.PciProjectAdditionalIpService.getAssociatedInstance(
          this.projectId,
          this.ip.region,
        ),
      })
      .then(({ instances, privateNetworks, associatedInstance }) => {
        this.associatedInstance = associatedInstance?.map(
          ({ associatedEntity }) => {
        ...
        this.instances = this.associatedInstance
          ? this.instances.filter(
              (instance) => !this.associatedInstance.includes(instance.id),
            )
          : this.instances;
