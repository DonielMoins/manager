import { checkFeatureAvailability } from '@/api/features';
...
export async function fetchBillingData(serviceName: string, shell: OvhContextShellType): Promise<BillingData> {
  ...
  const [
    ...
    checkFeatureAvailability('billing:management', 'dedicated'),
    checkFeatureAvailability('contact', 'dedicated'),
    checkFeatureAvailability('contact:management', 'dedicated'),
  ]);
